<?php 
	class Pwd_applications_table extends CI_Model {
	    ...
	    var $column_order = array(null,'applicant_name','applicant_email_id','applicant_mobile_no','applicant_address','letter_no','letter_date','company_name','landline_no','contact_person','days_of_work',null,null,null,null);

		//set column field database for datatable searchable 
	    var $column_search = array('applicant_name','applicant_email_id','applicant_mobile_no','applicant_address','letter_no','letter_date','company_name','landline_no','contact_person','days_of_work');
		...
		public function getAllApplicationsDetailsById($id = null) {
			...
			$query = $this->db->select($select_str)
							...
							->where(array('pwd.is_deleted'=>'0','pwd.id'=>$id))
							->get()
							->row_array();
			if($query) {
				return $query;
	   ...
	   // 		(SELECT role_id FROM permission_access pa WHERE pa.dept_id = 1 AND pa.status = 1 AND pa.role_id > IF(last_approved_role_id IS NULL,'0',last_approved_role_id) ORDER BY access_id ASC LIMIT 1) AS acessable_role_id");
	   
	       $this->db->select("pwd_applications.*, (SELECT company_name FROM company_details WHERE company_id = pwd_applications.company_name) company_name,
	    		(SELECT role_id FROM application_remarks WHERE application_remarks.app_id = pwd_applications.app_id ORDER BY id DESC LIMIT 1) AS last_approved_role_id,
	    		(SELECT role_id FROM permission_access pa WHERE pa.dept_id = 1 AND pa.status = 1 AND pa.role_id > IF(last_approved_role_id IS NULL,'0',last_approved_role_id) ORDER BY access_id ASC LIMIT 1) AS acessable_role_id,(SELECT COUNT(*) FROM latter_generation lg WHERE lg.app_id = pwd_applications.app_id AND lg.status = 1 AND lg.latter_type_id = 2) AS permition_latter_count,(SELECT COUNT(*) FROM payment py WHERE py.dept_id = 1 AND py.app_id = pwd_applications.app_id AND py.approved_by = 0 AND py.is_deleted = 0 AND py.status = 1) AS payment_done");
...
// 		}
		...
		public function getUserData($searchVal = null, $fromDate = null, $toDate  = null, $i = null, $rowperpage = null, $columnIndex = null, $columnName = null, $columnSortOrder = null, $user_id = null){
			...
			$dateCond = "";
			if($fromDate != '' && $toDate != ''){
				$dateCond = "WHERE date(pd.created_at) >= '".$fromDate."' AND date(pd.created_at) <= '".$toDate."'";
			}
			...
			if($searchVal != ''){
				...
				$totalCount = $this->db->query("SELECT COUNT(pd.id) cnt_id FROM (SELECT pwd.*, (SELECT rt.role_title FROM application_remarks ar INNER JOIN roles_table rt ON ar.role_id = rt.role_id AND rt.status = '1' WHERE ar.app_id = pwd.id AND ar.status = '1' ORDER BY ar.id DESC LIMIT 1) file_status FROM `pwd_applications` pwd WHERE pwd.user_id = '".$user_id."' AND pwd.is_deleted = '0') pd ".$dateCond." (pd.application_no like '%".$searchVal."%' OR pd.company_name like '%".$searchVal."%' OR pd.road_name like '%".$searchVal."%' OR pd.file_status like '%".$searchVal."%')")->result_array();

				$result = $this->db->query("SELECT pd.*, IF(pd.file_status IS NULL, 'Awaiting', pd.file_status) approved_by, (SELECT company_name FROM company_details WHERE company_id = pd.company_name) company_name FROM (SELECT pwd.*, (SELECT rt.role_title FROM application_remarks ar INNER JOIN roles_table rt ON ar.role_id = rt.role_id AND rt.status = '1' WHERE ar.app_id = pwd.id AND ar.status = '1' ORDER BY ar.id DESC LIMIT 1) file_status FROM `pwd_applications` pwd WHERE pwd.user_id = '".$user_id."' AND pwd.is_deleted = '0') pd ".$dateCond." (pd.application_no like '%".$searchVal."%' OR pd.company_name like '%".$searchVal."%' OR pd.road_name like '%".$searchVal."%' OR pd.file_status like '%".$searchVal."%')")->result_array();

			}else{
				
				$totalCount = $this->db->query("SELECT COUNT(pd.id) cnt_id FROM (SELECT pwd.*, (SELECT rt.role_title FROM application_remarks ar INNER JOIN roles_table rt ON ar.role_id = rt.role_id AND rt.status = '1' WHERE ar.app_id = pwd.app_id AND ar.status = '1' ORDER BY ar.id DESC LIMIT 1) file_status FROM `pwd_applications` pwd WHERE pwd.user_id = '".$user_id."' AND pwd.is_deleted = '0') pd ".$dateCond." ORDER BY ".$columnName." ".$columnSortOrder)->result_array();
		...
		public function create_joint_visit($insert_stack = null, $appID = null)
		{
			if(!empty($insert_stack)){
				$updates = $this->db->where(array('app_id' => $appID, 'type' => '1'))
						 ->update("joint_visit_extentions", array('is_deleted' => '1'));
				if($updates){
		...
		public function getExtensionId($appId = null){
			$this->db->select('*');
			$this->db->from('joint_visit_extentions');
			$this->db->where(['app_id'=>$appId,'type'=>2,'status'=>1]);
			$this->db->order_by("id","desc")->limit(1);
