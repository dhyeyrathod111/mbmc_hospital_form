<?php 
	class Qualification_master_table extends CI_Model {
		
		function __construct() {
	        // Set table name
	        $this->table = 'qualification_master';
	    ...
	    public function getAllQualification() {
	 	...
	 	public function getQualDetailsById($qual_id = null) {
	 		// echo'<pre>';print_r($road_id);exit;
			$query = $this->db->select('*')
							->from($this->table)
							->where(array('is_deleted'=>'0','qual_id'=>$qual_id))
							->get()
							->row_array();
			if($query) {
				return $query;
		...
		public function update($data = null,$qual_id = null) {
			if($data != null) {
				$result = $this->db
