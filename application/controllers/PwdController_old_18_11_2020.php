...
class PwdController extends Common {
	...
	public function save()
	{
        ...
        if ($image_upload_error == '') {
        	if (!empty($this->input->post('id')) && $this->input->post('id') != '') {
        		...
        		unset($datastack['status']);unset($datastack['app_id']);unset($datastack['user_id']);
	...
	public function get_lists()	{
        ...
        foreach($pwdList as $pwd){
            ...
            if ( $pwd['permition_latter_count'] != 0 && empty($pwd['joint_visit_creater_id']) && $pwd['file_closure_status'] == 0) {
			...
			if ($pwd['payment_done'] != 0 && $pwd['file_closure_status'] == 0) {
			...
			if ($pwd['extention_requested'] == 1 && $pwd['file_closure_status'] == 0) {
				$extentionOption = "<a data-microtip-position='top' role='tooltip' aria-label='Extention Request' application_id=".$pwd['id']." class='extention_approval_selecter' href='javaScript:void(0)'><i class='fas fa-arrow-alt-circle-right fa-lg' application_id=".$pwd['id']."></i></a>";
			} else {
				$extentionOption = "";
			}

			if ($pwd['file_ableto_close'] != 0 && $pwd['file_closure_status'] == 0) {
				$fileCloseOption = '<a href="javaScript:void(0)"><i application_id='.$pwd['id'].' class="fas fa-power-off fa-lg file_close_modal_selecter"></i></a>';
			} else {
				$fileCloseOption = '';
			}
            ...
            if ( file_exists(FCPATH.'uploads/demand_note/demand_note_'.$pwd['id'].'.pdf') ) {
            	$Generate_Letter = "<a class='btn btn-info btn-sm' href='".base_url('uploads/demand_note/demand_note_'.$pwd['id'].'.pdf')."' style='font-size: small'>Demand note</a>";
            } else {
            ...
            $data[] = array($i,$application_no, $applicant_name, $applicant_email_id, $applicant_mobile_no,$refrence_column,
            	$company_name, $days_of_work, $Generate_Letter ,$remarks,$status,$pwd['created_at'],$action,$file_closer_class);
        }
	...
	public function getUserApplicationList(){
		...
		if(!empty($appData)){
			...
			foreach($appData['result'] as $keyData => $valueData){
				...
				if (!empty($accessExtention) && $accessExtention->latter_type_id == 2) {
					$extentionOption = "<a class='extention_selecter' data-microtip-position='top' role='tooltip' aria-label='Create extension' href='javascript:void(0)'> <i class='fas fa-arrow-alt-circle-right fa-lg' application_id=".$valueData['id']."></i> </a>";
				} else {
					$extentionOption = "";
				}
				...
				if ($valueData['dispatched_refrence_number'] != 0) {
				...
				$data[] = array('sr_no' => $srNo,'application_no' => 'MBMC-00000'.$valueData['app_id'], 'company_name' => $valueData['companys_name'], 'road_name' => $valueData['road_name'],'data_added' => $valueData['created_at'], 'approval_status' => $valueData['approved_by'], 'action' => '<a aria-label="Edit" data-microtip-position="top" role="tooltip" href="'.base_url().'pwd/edit/'.base64_encode($valueData['id']).'" class="nav-link-icon"><i class="nav-icon fas fa-edit"></i>
					...
					<a data-microtip-position="top" role="tooltip" aria-label="Delete" application_id="'.$valueData['id'].'" href="'.base_url().'pwd/delete/'.base64_encode($valueData['id']).'" class="delete_user_application '.$is_deleted.' "><i class="nav-icon fas fa-trash"></i></a> '.$extentionOption.$joint_visit_new_app);
				$srNo++;
			}
		}else{
	...
	public function joint_visit_process()
	{
		...
		$this->data['file_url'] = base_url('letters/')."jointvisit_letter/".base64_encode($this->data['application']->id)."/".base64_encode($this->data['application']->app_id);
		...
		if ($this->pwd_applications_table->create_joint_visit($insert_array, $appID)) {
			...
			$createLatter = array(
				'dept_id' => 1,'app_id' => $appID,'latter_type_id' => $latter_info->id,'status' => TRUE,
			);
	...
	{
		$app_ai_id = base64_decode($this->input->post('app_ai_id'));
		$this->data['application'] = $this->pwd_applications_table->getApplicationByID($app_ai_id);
		$insert_array = array(
			'date' => $this->security->xss_clean($this->input->post('ext_date')),
			'to_date' => $this->security->xss_clean($this->input->post('ext_to_date')),
			'app_id' => $this->security->xss_clean($this->data['application']->id),
	...
	public function extention_approvel_datafetch()
	{
		...
		if (!empty($application)) {
			$extention_data = $this->pwd_applications_table->getExtentionInfoByAppID($application->id);
			if (!empty($extention_data)) {
				$this->response['status'] = TRUE;
				$this->response['extention_payload'] = $extention_data;
			} else {
				$this->response['status'] = FALSE;
				$this->response['message'] = "Sorry, we have to face some technical issues please try again later.";
			}
	...
	public function extention_approvel_process()
	{
		...
		$application = $this->pwd_applications_table->getApplicationByID($this->input->post('pwd_app_id'));
	...
	public function create_refrence_number()
	{
        ...
        if ($postData['is_close'] == 0) {
        	...
        	$this->response['status'] = FALSE;
        	$this->response['message'] = "No refrence application genrated joint visit has been closed";
        }
        return $this->output->set_status_header(200)->set_content_type('application/json')->set_output(json_encode($this->response));
	}
