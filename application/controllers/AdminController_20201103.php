    ...
    public function forgot_password_process()
    {
        ...
        if ($this->form_validation->run()) {
            ...
            if (!empty($user_data)) {
                ...
                $email_response = $this->email_trigger->sendMail($email_stack);
    ...
    public function process_payment()
    {
        ...
        if ($this->form_validation->run() && $document_response->status == TRUE) {
            ...
            if ($this->pwd_applications_table->createPayment($insertStack)) {
                $this->response['status'] = TRUE;
                $this->response['message'] = 'We have been received your payment successfully..!!!';
                if ($this->authorised_user['is_user'] == 0) {
                    $this->response['redirect_url'] = base_url('pwd/pwduserlist');
                } else {
                    $this->response['redirect_url'] = base_url('pwd');
                }
            } else {
                $this->response['status'] = FALSE;
                $this->response['message'] = 'Sorry, we have to face some technical issues please try again later.';
            ...
            if (validation_errors() != '') {
                ...
                $this->response['message'] .= strip_tags($document_response->error);
            } else {
                $this->response['message'] = 'Sorry, we have to face some technical issues please try again later.';   
            }
        }
