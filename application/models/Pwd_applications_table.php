	...
	class Pwd_applications_table extends CI_Model {
	    ...
	    public function getAuthorityData($count=FALSE,$postData)
	    {
	    	...
	    	switch ($approval_status) {
	    		case 'new':
			    	...
			    	$condition .= " AND pwd_data.is_deleted = 0 AND pwd_data.file_closure_status = 0";
	    			break;
	    		case '0':
    			$condition .= ' AND (pwd_data.acessable_role_id <= '.$this->authorised_user['role_id'].' OR pwd_data.last_approved_role_id >= '.$this->authorised_user['role_id'].') AND pwd_data.is_deleted = 0 AND pwd_data.file_closure_status = 0';
	    			break;
	    		case '2':
	    			if ($final_role_id[0]['role_id'] == $this->authorised_user['role_id']) {
	    				$condition .= ' AND pwd_data.acessable_role_id IS NULL AND pwd_data.file_closure_status = 0';
	    			} else {
	    				$condition .= ' AND (pwd_data.acessable_role_id > '.$this->authorised_user['role_id']." OR pwd_data.acessable_role_id IS NULL) AND pwd_data.file_closure_status = 0";
	    			}
	    			break;
	    		case '3':
	    			$condition .= ' AND pwd_data.is_deleted = 1 AND pwd_data.last_approved_role_id = '.$this->authorised_user['role_id']." AND pwd_data.file_closure_status = 0";
	    			break;
	    		case '4':
	    				$condition .= " AND pwd_data.file_closure_status = 1 ";
	    			break;
	    		default:
	    			break;
	    	}
	    	...
	    	if ($postData['permission_type'] != 0) {
	    		$condition .= " AND pwd_data.permission_type = ".$postData['permission_type'];
	    	}
		...
		public function getUserData($searchVal = null, $fromDate = null, $toDate  = null, $i = null, $rowperpage = null, $columnIndex = null, $columnName = null, $columnSortOrder = null, $user_id = null){
			...
			if($searchVal != ''){
				...
				if (substr($searchVal, 0, 10) == 'MBMC-00000') : $searchVal = ltrim($searchVal,"MBMC-00000"); endif;
		...
		public function create_joint_visit($insert_stack = null, $appID = null)
		{
			if(!empty($insert_stack)){
				$updates = $this->db->where(['app_id'=>$appID,'type'=>1])->update("joint_visit_extentions", array('is_deleted' => '1'));
				if($updates){
		...
		public function create_extention($insertStack)
		{
			if (!empty($insertStack)) {
				$this->db->where(['app_id'=>$insertStack['app_id'],'type'=>2])->update('joint_visit_extentions',['is_deleted'=>1]);
				return $this->db->insert('joint_visit_extentions',$insertStack);
			} else {
				return FALSE;	
			}
		}
		...
		{
			$this->db->select('*');
			$this->db->from('latter_generation lg');
			$this->db->join('latter_table lt','lg.latter_type_id = lt.id');
			$this->db->where(['lg.app_id'=>$app_id,'lt.latter_key'=>$lattter_key,'lg.is_deleted'=>0]);
			return $this->db->get()->row();
		}
		public function getExtentionInfoByAppID($app_id)
		{
			...
			$this->db->where(['app_id'=>$app_id,'type'=>2,'status'=>1,'is_deleted'=>0]);
			$this->db->order_by("id","desc")->limit(1);
			return $this->db->get()->row();
		}
		public function updateJointVisitExtention($update_stack,$extention_id)
		{
			return $this->db->where(['id'=>$extention_id,'is_deleted'=>0])->update('joint_visit_extentions',$update_stack);
		}
		...
		public function getJointVisitByRefrence($refrence_number)
		{
			...
			$this->db->order_by("pa.id","ASC")->limit(1);
			return $this->db->get()->row();	
		}
		...
		public function getRefundableAmountByAppID($application_id)
		{
			$this->db->select('SUM(security_deposit) AS refundable_amount');
			$this->db->from('road_information_pwd');
			$this->db->where(['pwd_app_id'=>$application_id,'status'=>1]);
			return $this->db->get()->row();
		}
		public function create_application_close($insertStack)
		{
			return $this->db->insert('closed_application_table',$insertStack);
		}
		public function getApprovedExtention($app_id)
		{
			$this->db->select('*');
			$this->db->from('joint_visit_extentions');
			$this->db->where(['app_id' => $app_id,'type'=>2,'status' => 2,'approved_by' != 0]);
			return $this->db->get()->row();
		}
		public function getPreviousAllExt($app_id)
		{
			$this->db->select('*');
			$this->db->from('joint_visit_extentions');
			$this->db->where(['app_id' => $app_id,'type'=>2]);
			$this->db->where('status != ',1);
			return $this->db->get()->result();
		}
		public function getAllRefrenceAppByAppid($referenceno)
		{
			$this->db->select('*');
			$this->db->from('pwd_applications');
