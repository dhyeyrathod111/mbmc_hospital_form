...
// Dhyey Rathod start
    ...
    const load_user_apps_datatable = props => {
        var dataTable = $('#lab_userapps_table').DataTable({
            "processing": true,
            "serverSide": true,
            "autoWidth" : false,
            "order": [],
            ...
            "ajax": {
                url: base_url + 'lab/datatable_userapplist',
                type: "POST",
                data: {
    ...
    $(document).on("click",".payment_approvel_btn",event => { 
        ...
        $.ajax({
            type: "POST",
            url: base_url + 'lab/payment_approvel_modal',
            data:{app_id:app_id},
            success: response => {
                if (response.status == true) {
...
$(document).on("click",".payment_request_btn",event => { event.preventDefault();
    ...
    $.ajax({
        type: "POST",
        url: base_url + 'lab/payment_reqeust_popup',
        data:{app_id:app_id},
        success: response => {
            if (response.status == true) {
...
$(document).on("click",".inspection_form_btn",event => { event.preventDefault();
    ...
    $.ajax({
        type: "POST",
        url: base_url + 'lab/inspection_form_display',
        data: {app_id:app_id},
        success: response => {
            if (response.status == true) {
...
const userListDataTable = () => {
    var lab = $('#lab_userapp').DataTable({
        ...
        "ajax": {
            url: base_url + '/authuser/get_userlab',
            type: "POST",
        },
        "bDestroy": true,
    });
...
$(document).on('ready',()=>{
    ...
    $("#application_date_lab").datepicker({ minDate: 0 , dateFormat: 'yy-mm-dd',autoclose: true});
    $("#Certificate_expirydate").datepicker({ minDate: 0 , dateFormat: 'yy-mm-dd',autoclose: true});
  ...
  $(document).on('click','.status_button',function(){
    ...
    if(status !='') {
        ...
        $.ajax({
            type: 'POST',
            url: base_url +'lab/get_application_status',
            dataType: "Json",
            data:{'dept_id':dept_id,'status':status,'user_id':user_id,'role_id':role_id,'app_id':app_id,'lab_id':lab_id},
            success: response => {
                if (response.status) {
