...
$(document).on('click','.file_close_modal_selecter', Event => { Event.preventDefault();
    ...
    $('.file_close_selecter').attr("application_id",application_id);
...
$(document).on('click','#refrence_order', Event => { Event.preventDefault();
	...
	$.ajax({
        ...
        error: response => {
        	console.log(response);swal("Success..!!!","Sorry, we have to face some technical issues please try again later.", "error");
        }
    });
});
...
$(document).on("click",".extention_approval_selecter",Event => {
	...
	$.ajax({
        ...
        data:{app_id:app_ai_id},
        success: response => {
        	if (response.status == true) {
        		$('#approvel_date_selecter').text(response.extention_payload.date);
        		$('#approvel_remark_selecter').text(response.extention_payload.description);
        		$('#extention_id').val(response.extention_payload.id);
        		$('#pwd_app_id').val(response.extention_payload.app_id);
...
$("#extention_form").validate({
    ...
    submitHandler: function(form) {
		...
		if (To_date <= From_date) {
			swal("Error..!!!", "Please Enter Correct Date Range.", "error"); return ;
		}
 ...
 //            return;
	...
	$("#pwd-form").validate({
		...
		submitHandler: function (form, e) { 
			...
			if ($(".company_designation_validation")[0].value == $(".company_designation_validation")[1].value) {
				...
				swal("Good job!", "designation of the Comopany Head and Designation Of Assistant is not same", "error");
				return ;
			} 
	...
	$(document).on('click', '.status_button', function () {
		...
		if (status != '') {
			...
			if (checkDeficitLIB(pwd_id) == 0 && approvel_access_status && !is_rejected_app.status) {
